...
namespace Inventory.Package1
{
    public class GetFilterContains
    {
        ...
        private static IQueryable<V2Outbill> _V2Outbill(IQueryable<V2Outbill> query, string col, string f1)//以  f1 替代了 2,3 等
        {
            if (string.IsNullOrWhiteSpace(f1)) //如果沒有值, 不處理, 簡單明瞭
                return query;
        ...
        public static IQueryable<V2Outbill> V2Outbill(IQueryable<V2Outbill> query, IBaseFiltersV2 f)
        {
       
            for (int i = 0; i < f.FilterContainsCol.Length; i++)
                if (f.FilterContainsCol[i] != null)//表示有值
                    query = _V2Outbill(query, f.FilterContainsCol[i], f.FilterContains[i]);
